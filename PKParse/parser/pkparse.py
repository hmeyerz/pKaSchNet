...
import edlib
import os
from utils import constants, inputs_directory

#data_dir = os.getcwd()
data = constants() #TODO can make problems a fn same name as var but didnt for me


fullcode, code, three_to_one, elements,  cof, ligands, amber = data["titratables_full"], data["titratables_short"], data["aa_code"], data["elements"], data["cofactors"], data["ligands"],data["amber_sites"]
#cnbrs = data["disulfide_nn"]
cof_upper = [s.upper() for s in cof.keys()] #added, since it wasnt gonna find a residue named Mg.
lig=np.append(ligands,cof_upper)
save_dir = inputs_directory() + "/"



class parser():
    def __init__(self, gzipped_pdb, ref_pdb, save_dir=save_dir):

        self.path    = gzipped_pdb
        self.pdb     = gzipped_pdb[-7:-3]
        self.targets =f"../../../src_data/targets/{self.pdb}.npz"
    ...
    def residue_map(self):
            ...
            **does not support files without:
            ...
            #save mapping for similiarity analysis 
            np.save(f"../../data/pkparse_OUTS/seqs/{self.pdb}.npy",cur_seqs)
        ...
        for line in lines:
            ...
            # skip hydrogens
                if not line[0:2].strip().startswith((b"H", b"D")): #here was the closing day bug. needs tuple 
                    ...
                    species.append(cof[line[-9:].split()[0][0:].upper()])
    ...
    def run(self):
        ...
        self.residue_map()
        if not self.mapping:
            print(f"no mapping for {self.pdb}")
            with gzip.open("dev/badparse.gz", "ab") as f:
            ...
            np.savez_compressed(f"../../data/pkparse_OUTS/res_maps/{self.pdb}.npz", map=list(self.mapping.items()))
...
class pkad2_parser():
    def __init__(self, gzipped_pdb, ref_pdb, save_dir=save_dir):
        ...
        #self.targets =f"../../src_data/targets/{self.pdb}.npz"
        self.save_dir = save_dir
        print("saving iputs to:",self.save_dir)
        self.ligands = lig
        self.ref_pdb = ref_pdb
    ...
    def run(self):
        ...
        self.residue_map()
        if not self.mapping:
            #if not "PKAD" in inputs_directory:
            with gzip.open("dev/pkad_badparse.gz", "ab") as f: #added
                f.write(self.pdb.encode())
                f.write(b"\n")
                return
            #else:
            ...
            np.save(f"../../data/pkparse_OUTS/PKAD/seqs/{self.pdb}.npy",self.mapping)
        ...
        if not flag: #if there was an issue in parsing the elements. should produce errors for Hydrogen, Deterium, N1+, O-, and EP, or Dy if somebody  parses pdb 4LLY.
            with gzip.open("dev/pkad_badparse.gz", "ab") as f:
                f.write(self.pdb.encode())
                f.write(b"\n")
                return
