...
elements = {
    b"D":1, b"H":1, b"LI":3, b"B":5, b"C":6, b"N":7, b"O":8,
    b"F":9, b"Na":11, b"MG":12, b"P":15, b"S":16, b"Cl":17,
    b"K":19, b"Ca":20, b"V":23, b"CR": 24, b"MN":25, b"FE":26, b"CO":27, b"NI":28,
    b"CU":29, b"ZN":30, b"SE":34, b"Br":35, b"MO":42, b"RU": 44, b"Cd":48,b"SN":50,
    b"I":53, b"CS":55, b"W":74, b"PT":78, b"Pb":82, b"BI": 83
}

cofactors={b"B":5, b"F":9,
...
def load_train_dir(data_dir):
    ...
    pdbs_, og, mdled = np.intersect1d(np.char.array([f[-11:-7] for f in pdbs]),np.char.array([f[-7:-3] for f in fixed_pdbs]), return_indices=True)
    #print(len(pdbs), len(fixed_pdbs),np.max(og),np.max(mdled))
