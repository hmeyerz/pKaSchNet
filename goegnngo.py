...
optimizer2 = torch.optim.Adam(list(net.parameters())+list(mha.parameters()), lr=.009, weight_decay=0.02)
...
train=set(paths).difference(set(val))
    ...
    try:
        ...
        for path in train:
            ...
            for z,x,y,id in zip(zs,xs,targets,ids):
                ...
                #x = x.unsqueeze(0)               # -> (1, n, 3) or (1, n, n)
                ...
                try:
                    ...
                    pk=torch.sum(torch.sum(torch.mean(mha(A(out[0][0].unsqueeze(2))), dim=0),dim=0))
                    loss = criterion(pk,torch.tensor(y))
                    loss.backward()
                    optimizer2.step()
                    with gzip.open("/home/jrhoernschemeyer/Desktop/data_prep/trainresults4.gz","a") as f:
        ...
        print("validating. last loss",loss.item())
        for path in val:
            mha.eval()
            net.eval()
            ...
            for z,x,y,id in zip(zs,xs,targets,ids):
                    try:
                        with torch.no_grad():
                            ...
                            #loss.backward()
                            #optimizer2.step()
                        with gzip.open("/home/jrhoernschemeyer/Desktop/data_prep/valresults4.gz","a") as f:
                            f.write(np.char.encode(str(y)))
                            f.write(b" ")
                            f.write(np.char.encode(str(pk.item())))
                            f.write(b" ")
                            ...
                            f.write(id)
                            f.write(b" ")
                            f.write(np.char.encode(path[-6:-2]))
                            f.write(b"\n")
                        #f.write(np.char.encode(path[-7:-3]))
                    except Exception as e:
                        print("except",pdb,id,e)
        ...
        print(e)
        print(pdb)
        torch.save(net.state_dict(),"/home/jrhoernschemeyer/Desktop/data_prep/nometals/egnn4_>200epoch_669structs")
        torch.save(mha.state_dict(),"/home/jrhoernschemeyer/Desktop/data_prep/nometals/mha4_>200epoch_669structs")#nettest")    
        continue
...
torch.save(net.state_dict(),"/home/jrhoernschemeyer/Desktop/data_prep/nometals/egnn4_200epoch_669structs")
torch.save(mha.state_dict(),"/home/jrhoernschemeyer/Desktop/data_prep/nometals/mha4_200epoch_669structs")#nettest")
