...
class EGNNBlock(nn.Module):
    def __init__(self, dim, depth, hidden_dim, dropout, num_positions, num_tokens, num_nearest_neighbors, norm_coors):
        super().__init__()
    ...
    def __init__(self, dim, depth, hidden_dim, dropout, num_positions, num_tokens, num_nearest_neighbors, norm_coors):
        ...
        self.blocks = nn.ModuleList([
            EGNNBlock(dim, depth, hidden_dim, dropout,
                      num_positions, num_tokens, num_nearest_neighbors, norm_coors)
            for _ in range(1)
        ])
...
class LearnableRBF(nn.Module):
    ...
    def pairwise_distances(self, centroids, coords):
        dist = torch.norm(centroids - coords, dim=-1)
        return dist

    
    def forward(self, centroids,coords):
        # dist: [B,N,N]
        dist = self.pairwise_distances(centroids,coords)
...
class TunableBlock(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        if self.enabled:
            ...
            # Identity passthrough — assume input is returned as output
            # For egnn: return args unchanged
            if len(args) == 2:
                return args  # (h_list, coords) passthrough
            elif len(args) == 1:
                return args[0]
            else:
                raise NotImplementedError("Unexpected input structure in TunableBlock")
