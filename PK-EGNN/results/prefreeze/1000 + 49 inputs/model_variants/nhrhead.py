...
modell=[egnn_net, protein_egnn, nconv,mha_layer,pred_head,pred_head2,conv] ####
...
def forward_residues(z_r, x_r):
    ...
    tok = h0.to(device)#torch.cat((r0, h0), dim=1)       # (R, dim+basis, N)
...
for epoch in range(epochs):
    ...
    for z, x, y, mask in train_loader:                 # z:(B,S,N)  mask:(B,S)
        ...
        with autocast():
                preds=conv(preds.to(device)).to(device)
