...
runid=datetime.datetime.now().strftime("%Y%m%d_%H%M%S") + "fixedseed" + "fixedrbf" + "1e-16"
...
modell=[egnn_net, protein_egnn, nconv,mha_layer,pred_head] ####
scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=.99, patience=0, cooldown=0, min_lr=1e-16, verbose=False)
epochs = 1000 # or whatever you like
...
def forward_residues(z_r, x_r):
    ...
    tok = h0#torch.cat((r0, h0), dim=1)       # (R, dim+basis, N)
...
for epoch in range(epochs):
    # ======== TRAIN ========
    egnn_net.train(); mha_layer.train(); conv.train(); nconv.train(); 
    pred_head.train(); protein_egnn.train(); #pred_head2.train(); 
    tr_losses = []

    for z, x, y, mask in train_loader:                 # z:(B,S,N)  mask:(B,S)
        #n=torch.randperm(len(mask.flatten()))

        # compress away padding →  (R, N), (R, N, 3), (R,)
        valid      = mask.view(-1)#[n]
        z_res      = z.view(-1, z.size(2))[valid].to(device)#[n]
        x_res      = x.view(-1, x.size(2), 3)[valid].to(device)#[n]
        y_res      = y.view(-1)[valid].to(device)#[n]
    ...
    print(f"Epoch {epoch:3d} | train MSE = {tmean:.4f}")
    tloss.append(tmean.item())
    #elapsed_min = (time.time() - t0) / 60
    
    # ======== VALID ========
    egnn_net.eval(); mha_layer.eval(); conv.eval(); nconv.eval(); 
    pred_head.eval(); protein_egnn.eval(); 
    vl_losses = []
...
torch.save(checkpoint, f"./checkpoints/last_benchmark_1049inputs_{runid}-checkpoint_{timestamp}.pt")
#torch.save(checkpoint, f"./test-checkpoint_{timestamp}.pt")
print(f"Saved checkpoint ./checkpoints/last_benchmark_1049inputs_{runid}-checkpoint_{timestamp}.pt", elapsed_min)
#os.system("wandb sync --include-offline --sync-all wandb")
