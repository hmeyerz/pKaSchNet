...
runid=datetime.datetime.now().strftime("%Y%m%d_%H%M%S") + "fixedseed" + "fixedrbf"
...
for epoch in range(epochs):
    # ======== TRAIN ========
    egnn_net.train(); mha_layer.train(); protein_egnn.train(); conv.train(); nconv.train();pred_head.train(); #pred_head2.train(); 
    tr_losses = []
    ...
    # ======== VALID ========
    egnn_net.eval(); mha_layer.eval(); protein_egnn.eval(); conv.eval(); nconv.eval();pred_head.eval(); pred_head2.eval(); 
    vl_losses = []
...
torch.save(checkpoint, f"./checkpoints/last_benchmark_1049inputs_{runid}-checkpoint_{timestamp}_norbf.pt")
#torch.save(checkpoint, f"./test-checkpoint_{timestamp}.pt")
