...
class Model(nn.Module):
    ...
    def __init__(self,c):
        ...
        self.prot  = EGNN(dim=inner_dim if c.aggregator=='nconv' else 1,update_coors=True,norm_coors=True, norm_feats=True, 
                          valid_radius=c.pegnn_radius).to(c.device) \
                     if c.use_prot else nn.Identity()
        self.rconv  = nn.Conv1d(1,1,1,padding=0).to(c.device) \
    ...
    def forward(self,z,x):
        ...
        if self.c.aggregator=='linear':
            ...
            if self.c.use_prot:
                preds = self.prot(preds.T.unsqueeze(-1),
        ...
        if self.c.use_conv:
            preds = self.rconv(preds.permute(2,0,1)).squeeze(0).T
