...
class ScaleShiftMACE(MACE):
    ...
    def forward(
        ...
        # Setup
        data["positions"].requires_grad_(True)
        data["node_attrs"].requires_grad_(True) #charges etc. atomic attributes
        num_graphs = data["ptr"].numel() - 1 #i think this is for the whole batch the number of atoms like [150 250 : mol2 ..]
        num_atoms_arange = torch.arange(data["positions"].shape[0])
        node_heads = (
            data["head"][data["batch"]]
            if "head" in data
        ...
        # Atomic energies
        node_e0 = self.atomic_energies_fn(data["node_attrs"])[
            num_atoms_arange, node_heads #node heads is 1, pka/energy?
        ]
        e0 = scatter_sum(
            src=node_e0, index=data["batch"], dim=0, dim_size=num_graphs
        )  # [n_graphs, num_heads]
        ...
        for interaction, product, readout in zip(
            ...
            node_feats, sc = interaction(
                node_attrs=data["node_attrs"], #atomic types?
                node_feats=node_feats, #atomic representation?
                edge_attrs=edge_attrs, #angular
                edge_feats=edge_feats, #radial interactions
                edge_index=data["edge_index"],
            )
            node_feats = product(
        ...
        # Concatenate node features
        node_feats_out = torch.cat(node_feats_list, dim=-1) #combines all features into 1
        # Sum over interactions ###
        node_inter_es = torch.sum(  ##HERE can change node_es_list to only the nodes that are tit res #the atom energy is the atoms e0 plus sum over interaction energy contributions from
            torch.stack(node_es_list, dim=0), dim=0
        )  # [n_nodes, ]
        node_inter_es = self.scale_shift(node_inter_es, node_heads)

        # Sum over nodes in graph
        inter_e = scatter_sum( #### initially dim=0
