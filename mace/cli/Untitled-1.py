    ...
    def forward(
       ...
       # Create a Mask for Titratable Sites
        ...
        for interaction, product, readout in zip(
            ...
            #cluster_assignment = torch.tensor([0, 1, 1])  # Oxygen is group 0, Hydrogens are group 1
            #coarse_grained_feats = scatter_mean(node_feats, torch.tensor([0, 1, 1]), dim=0)  # Shape [2, 64]
            ...
            for idx, count in torch.unique_consecutive(c1ei, return_inverse=False, return_counts=True, dim=None):
                ...
                if codes[idx]==0:
                    ...
                    #lengths = edge_index[edge_index[:, 0] == idx]
                    contributing_sites = [idx] + ints[:,1] #pals
                    for i in contributing_sites:
                    indices = torch.where(ints)[0]
                    new_node = sum(node_energies[i]/(lengths[j]) for i, j in zip(contributing_sites, indices)

                    new_node_idxs.append(idx)
                    for pair in edge_index[edge_index[:, 0] == idx]: #mask where first column = my site
                        newnode = 
                    
                    #can return counts and it tells about how dense it was
                    
                    new_node = sum((node_energies[site])\lengths[])
                                   
                #atom = edge_index[0] #should be an index like 1... N 
                if codes[atom] == 0: #doesnt contribute... MET
                    continue
                else: #it contributes
                    #get the set of edge indices connected to atom
                    new_node = sum((node_energies[site])\lengths[])


                if code == 1 or code==2:
                    atom_that_i_want= #H for the 2's, CA/O's/N's for the others
                    #add them up and divide by R = 5 or cutoff_radius
                    for edge, L in zip(edge_index[1,:], lengths): #[2, num_edges]
                    new_node = sum((node_energies[site])\lengths[])
                    
                
                titratable_site_i = 
                #specific H for tit sites
                #? for asp/glu, also ser and thr? 
                #CA if else
